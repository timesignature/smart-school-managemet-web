...
export default function CloudUploadStudentComponent({label='Photo',student}){

    const queryClient=useQueryClient()

    const mutation=useMutation(values=>http.post(`/student-img/${student.id}`,values).then(res=>res.data),{
        onSuccess:async(data)=>{
            student.avatar=data.avatar
            await queryClient.invalidateQueries(['student',data.id])
                    ...
                    student.avatar ? (
                        <div className="flex items-center space-x-3">
                            <div className='h-40 w-60'>
                                <img src={`${url}/${student.avatar}`} className='w-full h-full rounded-lg object-cover' alt=""/>
                            </div>
                        </div>
                    ) : (
                        <div className="flex items-center space-x-3">
